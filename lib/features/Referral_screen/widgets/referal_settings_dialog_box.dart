...
class _ReferalSettingsDialogBoxState extends State<ReferalSettingsDialogBox> {
  ...
  Future<void> _saveSettings() async {
    ...
    try {
      ...
      if (widget.existingSetting != null) {
        // Update existing setting - pass previous setting for comparison
        await ReferService.updateReferralSetting(
          widget.existingSetting!.id!,
          setting,
          widget.existingSetting!, // Pass the previous setting
        );
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
  ...
  ReferralSetting _createReferralSetting() {
    ...
    return ReferralSetting(
      ...
      updatedAt: DateTime.now(),
      version: widget.existingSetting?.version,
    );
  }
          ...
          children: [
            ...
            settingsDetailsFields(
              ...
              onChanged: (value) {
                setState(() {
                  selectedCommissionType = value!;
                  commissionController.clear(); // Clear when switching types
                });
              },
            ),
            ...
            settingsDetailsFields(
              ...
              onChanged: (value) {
                setState(() {
                  selectedDiscountType = value!;
                  discountController.clear(); // Clear when switching types
                });
              },
            ),
          ],
        ...
        SizedBox(
          ...
          child: TextFormField(
            ...
            decoration: InputDecoration(
              ...
              suffixText: selectedValue == 0 ? '%' : null,
            ),
            onChanged: (value) {
              if (selectedValue == 0) {
                ...
                String newValue = value.replaceAll(RegExp(r'[^0-9.]'), '');
                if (newValue.isNotEmpty) {
                  double? enteredValue = double.tryParse(newValue);
                ...
                // Amount validation (numbers and decimal)
                String newValue = value.replaceAll(RegExp(r'[^0-9.]'), '');
                controller.text = newValue;
                controller.selection = TextSelection.collapsed(
                  offset: controller.text.length,
